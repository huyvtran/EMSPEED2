			...
			if(twoDaysAgo.getTime() > $rootScope.user.date_votar){
				...
				confirmPopup.then(function(res) {
					if(res) {
						...
						localstorage.setObject('user', $rootScope.user);
						window.plugins.socialsharing.share("Ey, porque no pruebas Pocket Lines?", null, null, "xhttps://play.google.com/store/apps/details?id=es.bonda.pocketlines&hl=es");
...
//=================================================
// Favoritos
// Controler de la página de favoritos
// - Paradas guardadas
// - Más vistas
// - Recientes
//=================================================
.controller('Favoritos', function($scope, $rootScope, $ionicPopup, $ionicPlatform, $state, $ionicViewService, localstorage, FavTop){
	...
	// Añado la parada en favoritos o la elimino
	//=================================================
	$scope.parada = {id: Math.floor((Math.random() * 100) + 1), nombre: "Parada Test "+Math.floor((Math.random() * 100) + 1)};
	//console.log($scope.parada);
	$rootScope.favoritos = FavTop.toggleFavorito($scope.parada, $rootScope.favoritos);
	localstorage.setObject('favoritos', $rootScope.favoritos);
	*/
...
.controller('Avisos', function($scope, $rootScope, $ionicPlatform, InfoItinerario){
