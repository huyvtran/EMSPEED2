(function () {
    ...
    .factory('airportService', ['$http', '$q', function ($http, $q) {
        ...
        //<button class="btn btn-block btn-link" ng-click="chartType = 'bar'">Bar</button>
        //<button class="btn btn-block btn-link" ng-click="chartType = 'column'">Column</button>
        //<button class="btn btn-block btn-link" ng-click="chartType = 'pie'">Pie</button>
        //<button class="btn btn-block btn-link" ng-click="chartType = 'line'">Line</button>
        //<button class="btn btn-block btn-link" ng-click="chartType = 'area'">Area</button>
        //<button class="btn btn-block btn-link" ng-click="chartType = 'stepped'">Step</button>
        //<button class="btn btn-block btn-link" ng-click="chartType = 'table'">Table</button>
    ...
    /*******************************************/
    angularModule
    .factory('wtNotificationService', ['$http', '$q', function ($http, $q) {
        var get = function () {
    ...
    .directive('wtNotification', ['$rootScope', '$log', function ($rootScope, $log) {
        return {
            restrict: 'A',
            scope: {},
            replace: true,
            ...
            '    </div>' +
            '',
            controller: ['$scope', '$attrs', 'wtNotificationService', function ($scope, $attrs, wtNotificationService) {
                $scope.getAlerts = function () {
                    wtNotificationService.get().then(function (result) {
                        $scope.items = result;
                        //$scope.$apply();
                    }, function (reason) {
    ...
    /*******************************************/
    angularModule
    .factory('wtBarChartService', ['$http', '$q', function ($http, $q) {}])
    .directive('wtBarChart', ['$rootScope', '$log', function ($rootScope, $log) {
        return {
            restrict: 'A',
            scope: {},
            replace: true,
            ...
            '    </div>' +
            '',
            controller: ['$scope', '$attrs', 'airportService', function ($scope, $attrs, airportService) {
                $scope.chartType = 'bar';
    ...
    /*******************************************/
    angularModule
    .factory('wtPieChartService', ['$http', '$q', function ($http, $q) { }])
    .directive('wtPieChart', ['$rootScope', '$log', function ($rootScope, $log) {
        return {
            restrict: 'A',
            scope: {},
            replace: true,
    ...
    .directive("googleChart", function () {
        return {
            ...
            template: "<div></div>",
            scope: {
                type: "@",          // Chart type (area, bar, bubble, candle, column, combo, line, pie, scatter, stepped, or table).
                chartTitle: "@",    // Chart title.
                titleX: "@",        // Title for the X (category) axis.
                titleY: "@",        // Title for the Y (value) axis.
                minValue: "@",      // Minimum value to show on the Y axis.
                maxValue: "@",      // Maximum value to show on the Y axis.
                legend: "@",        // Whether to show a legend (by default, a legend is shown if there are more than one series).
                width: "@",         // Chart width in pixels.
                height: "@",        // Chart height in pixels.
                data: "="           // Chart data (array of arrays; first column contains the x values, others contain one series each - for most chart types).
            },
            link: function (scope, element, attrs) {

                // listen to changes in attributes and update the control
                var arr = ["type", "chartTitle", "titleX", "titleY", "minValue", "maxValue", "legend", "width", "data", "height"];
                ...
                function updateControl() {
                    ...
                    var firstRowIsData = !angular.isString(data[0][1]);

                    // convert data to array
                    var dataTable = google.visualization.arrayToDataTable(data, firstRowIsData);

                    // get chart object (function of chart type)
                    var el = element[0];
                    var type = scope.type ? scope.type : "line";
                    var chart = getChartObject(type, el);

                    // figure out whether to show a legend
                    var legend =
                        scope.legend ? scope.legend :
                        type == "pie" ? "right" :
                        type == "candle" || type == "table" ? "none" :
                        dataTable.getNumberOfColumns() > 2 ? "right" : "none";

                    // set chart options
                    var options = {};
                    options.legend = legend;
                    if (scope.chartTitle) options.title = scope.chartTitle;
                    if (scope.width) options.width = scope.width;
                    if (scope.height) options.height = scope.height;
                    options.chartArea = { top: "5%" };
                    //if (legend == "none" && type != "table") options.chartArea.width = "80%";
                    if (!options.hAxis) options.hAxis = {};
                    if (!options.vAxis) options.vAxis = {};
                    if (type != "bar") {
                        if (scope.titleX) options.hAxis.title = scope.titleX;
                        if (scope.titleY) options.vAxis.title = scope.titleY;
                        if (scope.minValue) options.vAxis.minValue = scope.minValue;
                        if (scope.maxValue) options.vAxis.maxValue = scope.maxValue;
                    } else {
                        if (scope.titleX) options.vAxis.title = scope.titleX;
                        if (scope.titleY) options.hAxis.title = scope.titleY;
                        if (scope.minValue) options.hAxis.minValue = scope.minValue;
                        if (scope.maxValue) options.hAxis.maxValue = scope.maxValue;
                    }
                ...
                function getChartObject(type, el) {
                    var gv = google.visualization;
                    switch (type.toLowerCase()) {
                        case "area": return new gv.AreaChart(el);
                        case "bar": return new gv.BarChart(el);
                        case "bubble": return new gv.BubbleChart(el);
                        case "candle": return new gv.CandlestickChart(el);
                        case "column": return new gv.ColumnChart(el);
                        case "combo": return new gv.ComboChart(el);
                        case "line": return new gv.LineChart(el);
                        case "pie": return new gv.PieChart(el);
                        case "scatter": return new gv.ScatterChart(el);
                        case "stepped": return new gv.SteppedAreaChart(el);
                        case "table": return new gv.Table(el);
                    }
